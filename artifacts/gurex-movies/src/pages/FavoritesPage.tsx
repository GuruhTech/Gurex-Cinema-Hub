...
export default function FavoritesPage() {
  ...
  const remove = (subjectId: string) => {
    savedFavorites.remove(subjectId);
    setItems(savedFavorites.get());
  };
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-white/40 mt-0.5">{items.length} title{items.length !== 1 ? "s" : ""} liked</p>
          </div>
        </div>
        ...
        {items.length === 0 ? (
          <div className="text-center py-24 text-white/30">
            ...
            <p className="text-sm mb-6">Heart your favorite movies and shows</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {items.map((item) => (
              <div key={item.subjectId} className="glass rounded-2xl overflow-hidden border border-white/5 group hover:border-red-500/20 transition-all">
