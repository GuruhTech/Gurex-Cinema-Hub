...
import { favorites, getImageUrl, formatVoteAverage, getRatingColor, getYear } from "@/lib/tmdb";
import type { WatchlistItem } from "@/lib/tmdb";

export default function FavoritesPage() {
  const [items, setItems] = useState<WatchlistItem[]>(() => favorites.get());
  const [filter, setFilter] = useState<"all" | "movie" | "tv">("all");

  const filtered = items.filter((i) => filter === "all" || i.media_type === filter);

  const remove = (id: number, mediaType: "movie" | "tv") => {
    favorites.remove(id, mediaType);
    ...
    <div className="min-h-screen pt-24 pb-16">
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground ml-4">{items.length} item{items.length !== 1 ? "s" : ""} favorited</p>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-24">
            ...
            <p className="text-xl font-semibold text-muted-foreground">No favorites yet</p>
            <p className="text-muted-foreground mt-2">Heart movies and shows you love</p>
            <Link href="/">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map((item) => (
              <div key={`${item.id}-${item.media_type}`} className="flex gap-3 p-4 rounded-xl bg-card border border-border group hover:border-red-400/40 transition-all duration-200">
