...
export const xcasper = {
  trending: {
    movies: (page = 0, perPage = 18) =>
      xFetch<SubjectListResponse>(
    ...
    all: (page = 0, perPage = 18) =>
      xFetch<SubjectListResponse>(`/trending?page=${page}&perPage=${perPage}`),
  },
  ...
  hot: () => xFetch<SubjectListResponse>(`/hot`),
  ranking: () => xFetch<SubjectListResponse>(`/ranking`),
  homepage: () => xFetch<HomepageResponse>(`/homepage`),
  ...
  search: {
    ...
    suggest: (keyword: string) =>
      xFetch<{ suggestList: SearchSuggest[] }>(
        `/search/suggest?keyword=${encodeURIComponent(keyword)}`
      ),
    popular: () => xFetch<{ keywords: string[] }>(`/popular-search`),
  },

  detail: (subjectId: string) =>
    xFetch<DetailResponse>(`/detail?subjectId=${subjectId}`),

  richDetail: (subjectId: string) =>
    xFetch<RichDetailResponse>(`/rich-detail?subjectId=${subjectId}`),
  ...
  play: (subjectId: string) =>
    xFetch<PlayResponse>(`/play?subjectId=${subjectId}`),

  stream: (subjectId: string) =>
    xFetch<PlayResponse>(`/bff/stream?subjectId=${subjectId}`),

  showbox: {
    movie: (id: string | number) =>
      xFetch<any>(`/showbox/movie?id=${id}`),
    streams: (id: string | number) =>
      xFetch<any>(`/showbox/streams?id=${id}`),
  },
};
...
export const savedWatchlist = storageList("xcasper_watchlist");
export const savedFavorites = storageList("xcasper_favorites");
export const savedWatchLater = storageList("xcasper_watch_later");
